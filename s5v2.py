...
def plot_all_bars(prices_in_float, exported_figure_filename):
	prices = list(map(int, prices_in_float)) # create a list of prices, maping the int() function onto each item in the price in float list
...
def create_bar_chart(price_groups, exported_figure_filename):
	...
	for group in price_groups:
		...
		ax.legend(label) # it looks like the legend() function is what you use to label the graph and we're passing in the labels list
...
def group_prices_by_range(prices_in_float):
	...
	for item in prices_in_float: # loop through the prices and put them into buckets depending on their price
		bucket = 0
...
price_groups = group_prices_by_range(prices_in_float)
create_bar_chart(price_groups, "_data/price_in_groups.png")
